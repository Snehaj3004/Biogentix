...
def predict_disease_risk(disease_code: str, symptoms: dict,
                          vitals: dict) -> dict:
    """
    ...
    if model_data is None:
        return None  # Will use rule engine as fallback

    model    = model_data['model']
    features = model_data['features']

    # Merge symptoms + vitals
    ...
    # Disease-specific prediction text
    predictions = {
    ...
    prediction   = predictions.get(disease_code, {}).get(pred_label, 'Assessment complete')
    confidence   = round(min(risk_score + 0.08, 1.0), 4)
